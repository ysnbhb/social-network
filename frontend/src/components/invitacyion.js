  ...
  return (
    <aside className={`activity-sidebar`} style={{ marginBottom: "20px" }}>
      ...
      <div className="users-list">
        {user.map((item) => (
          <Invtio key={item.id} user={item} id={id} />
        ))}
      </div>
    </aside>
  );
...
function Invtio({ user, id }) {
  const [invite, setInvite] = useState(false);
  const SendInv = async () => {
    if (invite) {
      return;
    }
    const response = await fetch(`${API_URL}/api/group/sendinvi`, {
      method: "POST",
      credentials: "include",
      headers: {
  ...
  return (
    <div className="activity-item">
      <div>
        <p>
          <strong>
            <Link
              className="link"
              href={{ pathname: `/profile/${user.nickname}` }}
            >
      ...
      <button className="btn btn-primary" onClick={() => SendInv()}>
        {invite ? "done" : "send invitation"}
      </button>
    </div>
  );
}
